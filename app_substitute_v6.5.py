...
# 設定頁面資訊
st.set_page_config(page_title="成德高中 智慧調代課系統 v7.1", layout="wide")
...
def clean_cell_text_v7(text):
    """
    清洗儲存格內容：移除亂碼、時間、無意義字詞
    """
    if not isinstance(text, str) or not text:
        return ""
    
    # 1. 移除特殊亂碼 (針對 114-2 課表出現的異常字元)
    text = re.sub(r'[کمکر]', '', text) 
    
    # 2. 移除時間格式 (如 08:00, 13:55) - 避免誤判為課程
    ...
    # 4. 移除常見雜訊
    noise_words = ["早自習", "午休", "上", "下", "午", "課程", "星期", "導師", "時間", "班級", "科目", "班期"]
...
def parse_pdf_v7(uploaded_file):
    ...
    # [關鍵修正] 擴充時間關鍵字，涵蓋 13:55, 14:45 等非整點時間
    time_keywords = {
        "1": ["第一節", "08:", "8:"], 
        "2": ["第二節", "09:", "9:"],
        "3": ["第三節", "10:"], 
        "4": ["第四節", "11:"],
        "5": ["第五節", "13:", "12:5", "12:"], # 涵蓋 13:55, 13:00, 12:55
        "6": ["第六節", "14:"],                # 涵蓋 14:45
        "7": ["第七節", "15:"], 
        "8": ["第八節", "16:"]
    }
        ...
        for i, page in enumerate(pdf.pages):
            ...
            if match:
                ...
                if len(raw_name) > 4 and "導師" in raw_name:
                     teacher_name = raw_name.replace("導師", "")[:3]
                else:
            ...
            for r_idx, row in enumerate(raw_table[:6]):
                for c_idx, cell in enumerate(row):
                    ...
                    for k, v in day_map_template.items():
                        if k in cell_str:
                            # 簡單防禦：避免重複覆蓋 (例如 '二三' 黏在一起時，優先保留第一個)
                            if c_idx not in col_map:
                                col_map[c_idx] = v
                                found_header = True
                if found_header and len(col_map) >= 3:
                    break
            
            # Fallback: 如果找不到標題，假設標準位置
            if not col_map:
            ...
            # Fallback Row Map: 如果完全抓不到時間關鍵字 (例如 OCR 亂碼嚴重)，啟用位置推算
            if not row_map:
                # 假設 Row 0=Header, Row 1=早自習, Row 2=第一節 ...
                # 這裡做一個保守估計
                for idx in range(len(raw_table)):
                    if idx == 2: row_map[idx] = "1"
                    elif idx == 3: row_map[idx] = "2"
                    elif idx == 4: row_map[idx] = "3"
                    elif idx == 5: row_map[idx] = "4"
                    elif idx == 6: row_map[idx] = "5" # 午休可能佔一行，這裡需視情況
                    elif idx == 7: row_map[idx] = "5" # 嘗試容錯
                    elif idx == 8: row_map[idx] = "6"
                    elif idx == 9: row_map[idx] = "7"
                    elif idx == 10: row_map[idx] = "8"
...
def show_schedule_popup(target_teacher, full_df, initiator_name, source_details, target_details):
    ...
    # [修復] 使用標準字串串接，避免 triple-quoted string 的縮排問題
    msg_template = (
...
def main():
    st.title("🏫 成德高中 智慧調代課系統 v7.1")
    
    if 'table_reset_key' not in st.session_state:
        st.session_state.table_reset_key = 0

    uploaded_file = st.sidebar.file_uploader("步驟 1: 上傳全校課表 PDF", type=["pdf"], key="uploader_v71")

    if uploaded_file:
        with st.spinner("正在進行智慧解析 (v7.1 強力修復版)..."):
            raw_data, teacher_classes_map = parse_pdf_v7(uploaded_file)
            
            if not raw_data:
                st.error("錯誤：無法從 PDF 中讀取有效課表。已嘗試所有解析模式，請確認檔案內容。")
                return
        ...
        with tab1:
            st.subheader("個別教師課表")
            t_select = st.selectbox("選擇教師", cached_teacher_list, key="t_sel_v71")
            if t_select:
        ...
        with tab2:
            ...
            q_day = c1.selectbox("星期", ["一", "二", "三", "四", "五"], key="q_d_v71")
            q_period = c2.selectbox("節次", [str(i) for i in range(1, 9)], key="q_p_v71")
            q_subject = c3.selectbox("科別篩選", ["全部"] + sorted(list(set(df['subject'].dropna()))), key="q_s_v71")
        ...
        with tab3:
            ...
            initiator = col_a.selectbox("誰要調課 (A老師)?", cached_teacher_list, key="swap_who_v71")
            swap_day = col_d.selectbox("A 想調開的星期", ["一", "二", "三", "四", "五"], key="swap_day_v71")
            swap_period = col_p.selectbox("A 想調開的節次", [str(i) for i in range(1, 9)], key="swap_per_v71")

            st.markdown("👇 **進階篩選條件**")
            cf1, cf2, cf3, cf4 = st.columns(4)
            filter_teacher = cf1.selectbox("還課教師 (指定B)", ["不指定"] + cached_teacher_list, key="fil_t_v71")
            filter_day = cf2.selectbox("還課星期", ["不指定", "一", "二", "三", "四", "五"], key="fil_d_v71")
            filter_period = cf3.selectbox("還課節次", ["不指定"] + [str(i) for i in range(1, 9)], key="fil_p_v71")
            filter_class = cf4.selectbox("還課班級", ["不指定"] + cached_class_list, key="fil_c_v71")
            ...
            if 'swap_results_v71' not in st.session_state:
                st.session_state.swap_results_v71 = None

            if st.button("🔍 搜尋雙向互換方案", key="btn_swap_v71"):
                ...
                if swap_options:
                    ...
                    st.session_state.swap_results_v71 = res_df
                else:
                    st.session_state.swap_results_v71 = pd.DataFrame()

            if st.session_state.swap_results_v71 is not None and not st.session_state.swap_results_v71.empty:
                st.success(f"找到 {len(st.session_state.swap_results_v71)} 個互換方案！請點擊查看 👇")
                
                dynamic_key = f"swap_table_v71_{st.session_state.table_reset_key}"
                
                event = st.dataframe(
                    st.session_state.swap_results_v71, 
                    hide_index=True, 
                ...
                if len(event.selection.rows) > 0:
                    row_data = st.session_state.swap_results_v71.iloc[event.selection.rows[0]]
            ...
            elif st.session_state.swap_results_v71 is not None and st.session_state.swap_results_v71.empty:
                if st.session_state.get('btn_swap_v71'):
